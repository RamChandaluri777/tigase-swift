...
open class JingleModule: XmppModuleBase, XmppModule {
    ...
    public static let XMLNS = "in:secure:vaditi:jingle:1";
    
    public static let MESSAGE_INITIATION_XMLNS = "in:secure:vaditi:jingle-message:0";
    ...
    open func bundle(from jingle: Element) -> [String]? {
        return jingle.findChild(name: "group", xmlns: "in:secure:vaditi:jingle:apps:grouping:0")?.mapChildren(transform: { (el) -> String? in
    ...
    public func initiateSession(to jid: JID, sid: String, contents: [Jingle.Content], bundle: [String]?, completionHandler: @escaping (Result<Void,XMPPError>)->Void) {
        ...
        if bundle != nil {
            let group = Element(name: "group", xmlns: "in:secure:vaditi:jingle:apps:grouping:0");
