...
open class MixModule: XmppModuleBaseSessionStateAware, XmppModule, RosterAnnotationAwareProtocol {
    func prepareRosterGetRequest(queryElem el: Element) {
        el.addChild(Element(name: "annotate", xmlns: "in:secure:vaditi:mix:roster:0"));
    }
    
    func process(rosterItemElem el: Element) -> RosterItemAnnotation? {
        guard let channelEl = el.findChild(name: "channel", xmlns: "in:secure:vaditi:mix:roster:0"), let id = channelEl.getAttribute("participant-id") else {
            return nil;
        }
        return RosterItemAnnotation(type: "mix", values: ["participant-id": id]);
    }
    
    public static let CORE_XMLNS = "in:secure:vaditi:mix:core:1";
    public static let IDENTIFIER = XmppModuleIdentifier<MixModule>();
    public static let ID = "mix";
    public static let PAM2_XMLNS = "in:secure:vaditi:mix:pam:2";
    ...
    open func join(channel channelJid: BareJID, withNick nick: String?, subscribeNodes nodes: [String] = ["in:secure:vaditi:mix:nodes:messages", "in:secure:vaditi:mix:nodes:participants", "in:secure:vaditi:mix:nodes:info", "in:secure:vaditi:avatar:metadata"], presenceSubscription: Bool = true, invitation: MixInvitation? = nil, messageSyncLimit: Int = 100, completionHandler: @escaping (Result<Iq,XMPPError>) -> Void) {
        guard let context = self.context else {
            completionHandler(.failure(.remote_server_timeout));
            return;
        }
    ...
    open func retrieveAffiliations(for channel: ChannelProtocol, completionHandler: ((PermissionsResult)->Void)?) {
        pubsubModule.retrieveOwnAffiliations(from: channel.jid, for: nil, completionHandler: { result in
            ...
            case .success(let affiliations):
                ...
                switch values["in:secure:vaditi:mix:nodes:info"] ?? .none {
                case .owner, .publisher:
                    permissions.insert(.changeInfo);
                default:
                    break;
                }
                switch values["in:secure:vaditi:mix:nodes:config"] ?? .none {
                case .owner, .publisher:
                    permissions.insert((.changeConfig));
                default:
                    break;
                }
                switch values["in:secure:vaditi:avatar:metadata"] ?? .none {
                case .owner, .publisher:
    ...
    open func retrieveParticipants(for channel: ChannelProtocol, completionHandler: ((ParticipantsResult)->Void)?) {
        pubsubModule.retrieveItems(from: channel.jid, for: "in:secure:vaditi:mix:nodes:participants", completionHandler: { result in
            switch result {
    ...
    open func publishInfo(for channelJid: BareJID, info: ChannelInfo, completionHandler: ((Result<Void,XMPPError>)->Void)?) {
        pubsubModule.publishItem(at: channelJid, to: "in:secure:vaditi:mix:nodes:info", payload: info.form().submitableElement(type: .result), completionHandler: { response in
            switch response {
            case .success(_):
                completionHandler?(.success(Void()));
            case .failure(let error):
    ...
    open func retrieveInfo(for channelJid: BareJID, completionHandler: ((Result<ChannelInfo,XMPPError>)->Void)?) {
        pubsubModule.retrieveItems(from: channelJid, for: "in:secure:vaditi:mix:nodes:info", completionHandler: { result in
            switch result {
            case .success(let items):
    ...
    open func retrieveConfig(for channelJid: BareJID, completionHandler: @escaping (Result<JabberDataElement,XMPPError>)->Void) {
        pubsubModule.retrieveItems(from: channelJid, for: "in:secure:vaditi:mix:nodes:config", limit: .lastItems(1), completionHandler: { result in
            switch result {
            case .success(let items):
    ...
    open func updateConfig(for channelJid: BareJID, config: JabberDataElement, completionHandler: @escaping (Result<Void,XMPPError>)->Void) {
        pubsubModule.publishItem(at: channelJid, to: "in:secure:vaditi:mix:nodes:config", payload: config.submitableElement(type: .submit), completionHandler: { response in
            switch response {
            case .success(_):
                completionHandler(.success(Void()));
            case .failure(let pubsubError):
    ...
    public enum AccessRule {
        ...
        var node: String {
            switch self {
            case .allow:
                return "in:secure:vaditi:mix:nodes:allowed";
            case .deny:
                return "in:secure:vaditi:mix:nodes:banned";
            }
        }
    }
    ...
    private func receivedPubsubItem(notification: PubSubModule.ItemNotification) {
        ...
        case "in:secure:vaditi:mix:nodes:participants":
        ...
        case "in:secure:vaditi:mix:nodes:info":
            switch notification.action {
            case .published(let item):
    ...
    open func retrieveHistory(fromChannel jid: BareJID, start: Date?, rsm: RSM.Query) {
        ...
        mamModule.queryItems(version: .MAM2, componentJid: JID(jid), node: "in:secure:vaditi:mix:nodes:messages", start: nil, queryId: queryId, rsm: rsm, completionHandler: { result in
            switch result {
...
open class MixInvitation {
    ...
    public convenience init?(from el: Element?) {
        guard el?.name == "invitation" && el?.xmlns == "in:secure:vaditi:mix:misc:0" else {
            return nil;
        }
    ...
    open func element() -> Element {
        return Element(name: "invitation", xmlns: "in:secure:vaditi:mix:misc:0", children: [
...
extension Message {
    open var mixInvitation: MixInvitation? {
        get {
            return MixInvitation(from: findChild(name: "invitation", xmlns: "in:secure:vaditi:mix:misc:0"));
        }
        set {
            element.removeChildren(where: { $0.name == "invitation" && $0.xmlns == "in:secure:vaditi:mix:misc:0"});
