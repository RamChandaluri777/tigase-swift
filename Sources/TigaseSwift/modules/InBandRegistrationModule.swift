...
extension StreamFeatures.StreamFeature {
    ...
    public static let ibrPreAuth = StreamFeatures.StreamFeature(name: "register", xmlns: "in:secure:vaditi:invite");
}
    ...
    open class AccountRegistrationTask {
        ...
        fileprivate func isStreamReady(streamFeatures: StreamFeatures) -> Bool {
            ...
            if preauthSupported && !preauthDone, let preauth: String = preauthToken {
                ...
                iq.to = JID(domain);
                iq.addChild(Element(name: "preauth", attributes: ["token": preauth, "xmlns": "in:secure:vaditi:pars:0"]));
