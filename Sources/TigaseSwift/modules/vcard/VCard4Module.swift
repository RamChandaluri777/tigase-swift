...
open class VCard4Module: XmppModuleBase, XmppModule, VCardModuleProtocol {
    ...
    fileprivate static let VCARD_XMLNS = "in:secure:vaditi:xml:ns:vcard-4.0";
...
extension VCard {
    ...
    public convenience init?(vcard4: Element?) {
        guard vcard4 != nil && vcard4!.name == "vcard" && vcard4!.xmlns == "in:secure:vaditi:xml:ns:vcard-4.0" else {
            return nil;
        }
        self.init();
    ...
    open func toVCard4() -> Element {
        let vcard = Element(name: "vcard", xmlns: "in:secure:vaditi:xml:ns:vcard-4.0");
