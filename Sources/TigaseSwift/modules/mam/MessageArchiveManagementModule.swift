...
open class MessageArchiveManagementModule: XmppModuleBase, XmppModule, Resetable {
    ...
    public static let MAM_XMLNS = "in:secure:vaditi:mam:1";
    public static let MAM2_XMLNS = "in:secure:vaditi:mam:2";
    ...
    open func process(stanza: Stanza) throws {
        for result in stanza.element.getChildren() {
            ...
            guard let messageId = result.getAttribute("id"), let queryId = result.getAttribute("queryid"), let forwardedEl = result.findChild(name: "forwarded", xmlns: "in:secure:vaditi:forward:0"), let query = dispatcher.sync(execute: { return self.queries[queryId]; }) else {
                return;
            }
            ...
            }).first, let delayEl = forwardedEl.findChild(name: "delay", xmlns: "in:secure:vaditi:delay") else {
                return;
            }
...
//            }
    ...
    public enum Version: String {
        case MAM1 = "in:secure:vaditi:mam:1"
        case MAM2 = "in:secure:vaditi:mam:2"
