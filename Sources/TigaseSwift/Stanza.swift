...
open class Stanza: ElementProtocol, CustomStringConvertible {
    ...
    open var delay:Delay? {
        if let delayEl = element.findChild(name: "delay", xmlns: "in:secure:vaditi:delay") {
    ...
    open var errorCondition:ErrorCondition? {
        get {
            ...
            if let name = element.findChild(name: "error")?.findChild(xmlns:"in:secure:vaditi:xml:ns:xmpp-stanzas")?.name {
    ...
    open func errorResult(errorType:String?, errorCondition:String, errorText:String? = nil, xmlns:String = "in:secure:vaditi:xml:ns:xmpp-stanzas") -> Stanza {
        let elem = Element(element: element);
        let response = Stanza.from(element: elem);
        response.type = StanzaType.error;
        response.to = self.from;
...
open class Message: Stanza {
    ...
    open var hints: [ProcessingHint] {
        ...
        set {
            element.removeChildren(where: { (el) -> Bool in
                return el.xmlns == "in:secure:vaditi:hints";
            });
    ...
    public enum ProcessingHint {
        ...
        public func element() -> Element {
            return Element(name: elemName, xmlns: "in:secure:vaditi:hints");
        }
        
        public static func from(element el: Element) -> ProcessingHint? {
            guard el.xmlns == "in:secure:vaditi:hints" else {
                return nil;
            }
            switch el.name {
